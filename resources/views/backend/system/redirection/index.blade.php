        ...
        <div class="card mb-4">
            ...
            <div class="card">
                <div class="card-datatable table-responsive">
                    <form method="get" action="{{ route('redirections.index') }}">
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                        ...
                                        <div class="dt-buttons btn-group flex-wrap">
                                            <div class="btn-group">
                                                ...
                                                @if(hasPermission('/'.strtolower($title).'/*','put'))
                                                    <a class="btn add-new btn-primary text-white" href="{{ route('redirections.create') }}">
                            ...
                            <table class="datatables-redirections table dataTable no-footer dtr-column" id="DataTables_Table_0">
                                <thead class="border-top">
                                <tr>
                                    <th>{{ __('SN') }}</th>
                                    <th>{{ __('Title') }}</th>
                                    <th>{{ __('Source Link') }}</th>
                                    <th>{{ __('Destination Link') }}</th>
                                    <th>{{ __('Status') }}</th>
                                    <th>{{ __('Action') }}</th>
                                </tr>
                                </thead>
                                            ...
                                            @if(hasPermission('/'.strtolower($title).'/*','put') || hasPermission('/'.strtolower($title).'/*','delete'))
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if(hasPermission('/'.strtolower($title).'/*','put'))
                                                            <a class="dropdown-item" href="{{ route('redirections.edit', $post->id) }}">
                                                                <i class="ti ti-pencil me-1"></i>{{ __('Edit') }}
                                                            </a>
                                                        @endif
                                                        @if(hasPermission('/'.strtolower($title).'/*','delete'))
                                                            <a href="#" class="dropdown-item delete-button" data-bs-toggle="modal" data-actionurl="{{ route('redirections.destroy', $post->id) }}" data-bs-target="#deleteModal">
