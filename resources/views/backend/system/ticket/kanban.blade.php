    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            <h5 class="card-header d-flex justify-content-between align-items-center flex-wrap">
                <span class="mb-2 mb-md-0">{{ $title }} - Kanban Board</span>
                <div class="d-flex gap-2 flex-wrap">
                    <a href="{{ route('tickets.index', $project->id) }}" class="btn btn-label-secondary btn-sm">
            ...
            <div class="card-body">
                ...
                <div class="alert alert-primary mb-4">
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        <div>
                            <strong><i class="ti ti-folder me-1"></i>{{ __('Project') }}:</strong> {{ $project->name }} ({{ $project->key }})
                        </div>
                        <div class="mt-2 mt-md-0">
                            <span class="badge bg-info">{{ $ticketStatuses->sum(function($s) { return $s->tickets->count(); }) }} Total Tickets</span>
                        </div>
                    </div>
                </div>

                <!-- Filters -->
                <form method="get" action="{{ route('tickets.kanban', $project->id) }}" class="mb-4">
                    <div class="row g-3">
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <button type="button" class="btn btn-label-secondary w-100" onclick="window.location.href='{{ route('tickets.kanban', $project->id) }}'">
                ...
                <div class="kanban-container" style="display: flex; gap: 15px; overflow-x: auto; padding-bottom: 20px;">
                    @foreach($ticketStatuses as $status)
                        <div class="kanban-column" style="min-width: 320px; background: #f8f9fa; border-radius: 10px; padding: 15px;">
                                ...
                                @foreach($status->tickets as $ticket)
                                    <div class="kanban-item" 
                                        ...
                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                            ...
                                            <div class="dropdown">
                                                <button type="button" class="btn btn-sm p-0" data-bs-toggle="dropdown" onclick="event.stopPropagation();">
                                                    <i class="ti ti-dots-vertical"></i>
                                                </button>
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item" href="{{ route('tickets.show', [$project->id, $ticket->id]) }}">
                                                        <i class="ti ti-eye me-1"></i>View Details
                                                    </a>
                                                    <a class="dropdown-item edit-ticket-btn" href="javascript:void(0);" data-ticket-id="{{ $ticket->id }}" data-bs-toggle="modal" data-bs-target="#editTicketModal">
                                                        <i class="ti ti-pencil me-1"></i>Quick Edit
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mt-2" style="font-size: 0.75rem; color: #697a8d;">
                                            ...
                                            @endif
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    ...
                                                    <li><a class="dropdown-item quick-assign" href="javascript:void(0);" data-ticket-id="{{ $ticket->id }}" data-assignee-id="">
                                                        <i class="ti ti-user-x me-2"></i>Unassigned
                                                    </a></li>
                                                    <li><hr class="dropdown-divider"></li>
                                                    @foreach($users as $user)
                                                        <li><a class="dropdown-item quick-assign {{ $ticket->assignee_id == $user->id ? 'active' : '' }}" href="javascript:void(0);" data-ticket-id="{{ $ticket->id }}" data-assignee-id="{{ $user->id }}">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea name="description" id="create_description" rows="4"
                                          class="form-control" placeholder="Describe the ticket..."></textarea>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="story_points" id="create_story_points" class="form-control" min="0" max="100" placeholder="0" />
                            </div>

                            <!-- Status (Active/Inactive) -->
                            <div class="col-md-6">
                                <label class="form-label w-100"><i class="ti ti-toggle-right me-1"></i>{{ __('Status') }}</label>
                                <div class="form-check-inline">
                                    <input type="radio" id="create_status1" name="status" value="1" checked class="form-check-input">
                                    <label for="create_status1" class="form-check-label">{{ __('Active') }}</label>
                                </div>
                                <div class="form-check-inline">
                                    <input type="radio" id="create_status2" name="status" value="0" class="form-check-input">
                                    <label for="create_status2" class="form-check-label">{{ __('Inactive') }}</label>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-0">
                    ...
                    @csrf
                                            ...
                                            <textarea class="form-control mb-2" id="new_comment" name="comment" rows="3" placeholder="Add a comment..."></textarea>
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <div class="form-text">Max 10MB per file. Multiple files allowed. Allowed: JPG, PNG, GIF, PDF, DOC, DOCX, TXT, XLSX, XLS</div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize Sortable for Kanban with better animation
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        if (oldCount) {
                            ...
                            oldCount.classList.add('badge-pulse');
                            setTimeout(() => oldCount.classList.remove('badge-pulse'), 300);
                        }
                        if (newCount) {
                            newCount.textContent = parseInt(newCount.textContent) + 1;
                            newCount.classList.add('badge-pulse');
                            setTimeout(() => newCount.classList.remove('badge-pulse'), 300);
                        }
    ...
    document.getElementById('createTicketForm').addEventListener('submit', function(e) {
        ...
        fetch('{{ route("tickets.store", $project->id) }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': csrfToken,
                'Accept': 'application/json'
    ...
    function updateTicketField(ticketId, fieldName, fieldValue) {
        ...
        // Fetch current ticket data first
        ...
        .then(response => {
            if (response.redirected) {
                ...
                loadTicketDetails(ticketId); // Reload details
                loadActivities(ticketId); // Reload activities
                return null;
            }
            return response.json();
        })
    ...
    document.querySelectorAll('.edit-ticket-btn').forEach(function(btn) {
    ...
    function loadTicketDetails(ticketId) {
            ...
            .then(data => {
                ...
                // Display reporter, created, updated
    ...
    function formatMentions(text) {
        ...
        // Create a safe copy of the text
        let result = text;
        ...
        // Track which mentions we've already replaced to avoid duplicates
        const replacedPositions = new Set();
        
        // First pass: Replace valid user mentions
        sortedUsers.forEach(user => {
            ...
            while ((startIndex = result.indexOf(mentionText, startIndex)) !== -1) {
                ...
                if (!replacedPositions.has(startIndex)) {
                    ...
                    // Create the link with proper escaping
    ...
    // Reload comments function (for after adding a comment)
    ...
    // Reload activities function (for after updates)
    ...
    document.getElementById('addCommentForm').addEventListener('submit', function(e) {
        ...
        // Create FormData from the form (includes file if present)
        const formData = new FormData(this);
        
        // Extract and add mentioned user IDs
    ...
    // Handle Quick Assign - Direct Click Handler
    document.body.addEventListener('click', function(e) {
        // Check if clicked element or its parent is a quick-assign link
        ...
        if (assignBtn) {
            ...
            console.log('Quick assign clicked:', { ticketId, assigneeId }); // Debug log
            ...
            // Update assignee via dedicated endpoint
    ...
    }, true); // Use capture phase to catch the event before it's stopped
});
</script>
<style>
