                    ...
                    <form method="get" action="{{ route('projects.index') }}">
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                        ...
                                        <div class="dt-buttons btn-group flex-wrap">
                                            <div class="btn-group">
                                                ...
                                                @if(hasPermission('/'.str_replace(' ','-',strtolower($title)),'post'))
                                                    <a class="btn add-new btn-primary text-white" href="{{ route('projects.create') }}">
                            ...
                            <table class="datatables table dataTable no-footer dtr-column" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                                ...
                                @endif
                                @foreach ($thisDatas as $projectKey => $project)
                                    <tr>
                                        <td>{{ $projectKey + 1 }}</td>
                                        <td>{{ $project->name }}</td>
                                        <td><span class="badge bg-label-info">{{ $project->key }}</span></td>
                                        <td>{{ $project->creator->name ?? 'N/A' }}</td>
                                        <td><span class="badge @if ($project->status) bg-label-primary @else bg-label-secondary @endif me-1">
                                                @if ($project->status)
                                                    {{ __('Active') }}
                                                @else
                                                    {{ __('Inactive') }}
                                            ...
                                            @if(hasPermission('/' . str_replace(' ','-',strtolower($title)) . '/*', 'put') || hasPermission('/' . str_replace(' ','-',strtolower($title)) . '/*', 'delete'))
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item" href="{{ route('tickets.index', $project->id) }}">
                                                            <i class="ti ti-ticket me-1"></i>{{ __('View Tickets') }}
                                                        </a>
                                                        @if(hasPermission('/' . str_replace(' ','-',strtolower($title)) . '/*', 'put'))
                                                            <a class="dropdown-item" href="{{ route('projects.edit', $project->id) }}"><i class="ti ti-pencil me-1"></i>{{ __('Edit') }}</a>
                                                        @endif
                                                        @if(hasPermission('/' . str_replace(' ','-',strtolower($title)) . '/*', 'delete'))
                                                            <a href="#" class="dropdown-item delete-button" data-bs-toggle="modal" data-actionurl="{{ route('projects.destroy', $project->id) }}" data-bs-target="#deleteModal">
