...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            ...
            <form class="card-body" action="{{ route('sms.update', $thisData->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <input value="{{ old('title', $thisData->title) }}" type="text" name="title" id="title"
                               class="form-control @error('title') is-invalid @enderror"
                               placeholder="SMS title">
                        <div class="invalid-feedback">{{ $errors->first('title') }}</div>
                    </div>

                    <!-- Receiver (Multi-select with custom input) -->
                    <div class="col-md-12">
                        ...
                        <select name="receiver[]" id="receiver"
                                class="form-control select2-multiple @error('receiver') is-invalid @enderror" multiple>
                            @php
                                $selectedReceivers = old('receiver', $thisData->receiver);
                            @endphp
                            @foreach(getAllPhoneNumbers() as $phoneNumber)
                                <option value="{{ $phoneNumber }}" {{ collect($selectedReceivers)->contains($phoneNumber) ? 'selected' : '' }}>
                                    {{ $phoneNumber }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea required name="message" id="message"
                                  ...
                                  rows="6" onkeyup="countCharacters(this)">{{ old('message', $thisData->message) }}</textarea>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                            @foreach(['draft', 'sent', 'send-now','failed', 'inbox'] as $status)
                                <option value="{{ $status }}" {{ old('status', $thisData->status) === $status ? 'selected' : '' }}>
                                    {{ ucfirst($status) }}
                                </option>
                            @endforeach
                        </select>
