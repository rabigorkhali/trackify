    ...
    Iconpicker.prototype.updateIcons = function (page) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            for (var j = 0; j < op.cols; j++) {
                ...
                var btn = $('<button class="btn ' + op.unselectedClass + ' btn-icon d-inline-flex align-items-center justify-content-center"></button>').hide();
    ...
    Iconpicker.prototype.updateIconsCount = function () {
        ...
        if(op.footer === true){
            var icons_count = [
                ...
                '       <span class="icons-count d-inline-block"></span>',
                '   </td>',
                '</tr>'
            ];
    ...
    Iconpicker.prototype.updatePagesCount = function () {
        ...
        if(op.header === true){
            ...
            for (var i = 0; i < op.cols; i++) {
                ...
                if (i === 0 || i === op.cols - 1) {
                    var arrow = [
                        '<button class="btn btn-arrow ' + ((i === 0) ? 'btn-previous' : 'btn-next') + ' ' + op.arrowClass + ' d-inline-flex align-items-center justify-content-center" value="' + ((i === 0) ? -1 : 1) + '">',
                    ...
                    td.attr('colspan', op.cols - 2).append('<span class="page-count d-inline-block"></span>');
                    tr.append(td);
                }
            }
                ...
                if($this.prop('tagName') === 'BUTTON'){
                    ...
                    $this.on('click', function(e) {
                        ...
                        if (!$this.data('bs.popover')) {
                            ...
                            // Add custom class after popover is inserted
                            $this.on('inserted.bs.popover', function() {
                                const popoverElement = document.querySelector('.popover');
                                if (popoverElement) {
                                    popoverElement.classList.add('iconpicker-popover');
                                }
                            });
                            ...
                            const popover = bootstrap.Popover.getInstance($this[0]);
                            popover.show();
                        }
                    });
                }
